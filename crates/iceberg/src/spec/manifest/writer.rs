...
mod tests {
    ...
    async fn test_preserving_status_added_normalizes_to_existing() {
        ...
        let manifest = Manifest::parse_avro(std::fs::read(&path).unwrap().as_slice()).unwrap();
        let out = &manifest.entries()[0];

        assert_eq!(
            out.status(),
            ManifestStatus::Existing,
            "Added must be normalized to Existing"
        );
        assert_eq!(
            out.snapshot_id(),
            entry.snapshot_id(),
            "snapshot_id must be preserved, not overwritten with writer's"
        );
        assert_eq!(
            out.sequence_number(),
        ...
        assert_eq!(
            out.file_sequence_number(),
            entry.file_sequence_number(),
            "file_sequence_number must be preserved"
        );
    }
    ...
    async fn test_preserving_status_deleted_stays_deleted() {
        ...
        assert_eq!(
            out.status(),
            ManifestStatus::Deleted,
            "Deleted must not be resurrected as Added"
        );
        assert_eq!(out.snapshot_id(), entry.snapshot_id());
        assert_eq!(out.sequence_number(), entry.sequence_number());
        assert_eq!(out.file_sequence_number(), entry.file_sequence_number());
